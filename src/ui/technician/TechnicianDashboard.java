...
public class TechnicianDashboard extends JFrame {
    ...
    // Only one nav panel — still track for consistency
    ...
    private void switchTab(String name, java.util.function.Supplier<JPanel> supplier) {
        ...
        if (btnAppts != null) btnAppts.repaint();
    ...
    private JButton createBellButton() {
        ...
        btnBell.addActionListener(e -> NotificationPanel.show(btnBell, technician, () -> {
    ...
    private JPanel buildSidebar() {
        ...
        if (spec != null && !spec.isEmpty()) {
            lblSpec = UITheme.mutedLabel("⚙️  " + spec);
            lblSpec.setAlignmentX(CENTER_ALIGNMENT);
        ...
        sidebar.add(roleRow);
        if (lblSpec != null) {
            JPanel specRow = new JPanel(new FlowLayout(FlowLayout.CENTER, 0, 0));
            specRow.setOpaque(false);
            specRow.setAlignmentX(Component.LEFT_ALIGNMENT);
            specRow.setMaximumSize(new Dimension(Integer.MAX_VALUE, 18));
            specRow.add(lblSpec);
        ...
        btnAppts = UITheme.sidebarButton("📋  My Appointments", () -> switchTab(PANEL_APPTS, () -> new TechMyAppointmentsPanel(technician)), activeAppts);
        ...
        JButton btnEdit = UITheme.sidebarButton("✏️  Edit Profile", () -> new EditProfileFrame(technician, () -> lblName.setText(technician.getName())).setVisible(true), dummy1);
